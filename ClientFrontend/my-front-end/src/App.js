...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/openai-chatbot", {
        user_input: input,
      ...
      <main>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label htmlFor="input">Type your question:</label>
            <input
              id="input"
              type="text"
    ...
    <div className="message-list">
      ...
      <ul>
        {messages.map((message, index) => (
          <li key={index}>
            <strong>{message.role === "user" ? "User" : "Response"}:</strong>{" "}
            {message.content.includes("```") ? (
              <pre className={message.role === "user" ? "user-code" : "chatgpt-code"}>
                <code>{message.content.replace(/```/gm, "")}</code>
              </pre>
            ) : (
